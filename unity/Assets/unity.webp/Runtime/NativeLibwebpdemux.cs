...
namespace unity.libwebp
{
    ...
    public static unsafe partial class NativeLibwebpdemux
    {
        ...
        public static void WebPDataInit(WebPData* webp_data)
        {
            if (webp_data != null)
            {
                // using unsafeutility instead of Unsafe.InitBlockUnaligned
                UnsafeUtility.MemClear(webp_data, sizeof(WebPData));
            }
        }
        ...
        public static int WebPDataCopy([NativeTypeName("const WebPData *")] WebPData* src, WebPData* dst)
        {
            ...
            if (src->bytes != null && (uint)src->size != 0)
            {
                ...
                //System.Buffer.MemoryCopy(src->bytes, dst->bytes, src->size, src->size);
                UnsafeUtility.MemCpy(dst->bytes, src->bytes, (uint)src->size);
                dst->size = src->size;
            }
